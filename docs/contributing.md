...
* Some shortcuts should only work when AltTab is open
  ...
  * Specific regression test: with hold shortcut = `Cmd` and `shortcutStyle` = `searchOnRelease`, do `Cmd`+`Tab`, release `Tab`, press `Escape`; the overlay must close. Regression caught in [#5616](https://github.com/lwouis/alt-tab-macos/issues/5616).
